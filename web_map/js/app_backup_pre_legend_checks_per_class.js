...
function classOpacityExpression() {
  return [
    "match",
    ["get", "classe_dinamica"],
    "non significativa", 0.08,
    "priorità ordinaria", 0.30,
    0.72
  ];
}
...
function renderClassLegend() {
  ...
  legend.innerHTML = `
    ...
    ${legendClassRow("priorità educativa scout")}
    ${legendClassRow("bacino scout alto")}
    ${legendClassRow("fragilità educativa alta")}
    ${legendClassRow("priorità ordinaria")}
    ${legendClassRow("non significativa")}
  `;
...
function appendLayerLegendRows() {
  ...
  const rows = [
    {
      label: "Esagoni",
      visible: esagoniVisible,
      swatch: '<span class="swatch" style="background:#c9c9c9"></span>',
      toggle() {
        esagoniVisible = !esagoniVisible;
      }
    },
    {
      label: "5 aree prioritarie selezionate",
      visible: top5Visible,
      swatch: '<span class="swatch top5"></span>',
      toggle() {
        top5Visible = !top5Visible;
      }
    },
    {
      label: "Gruppi scout Zona Varese",
      visible: gruppiVisible,
      swatch: '<span class="swatch circle" style="background:#6a3d9a"></span>',
      toggle() {
        gruppiVisible = !gruppiVisible;
      }
    },
    {
      label: "Gruppi AGESCI zone vicine",
      visible: gruppiVicineVisible,
      swatch: '<span class="swatch external-circle"></span>',
      toggle() {
        gruppiVicineVisible = !gruppiVicineVisible;
      }
    }
  ];
  ...
  rows.forEach(row => {
    ...
    el.className = "legend-row legend-direct-toggle";
    el.classList.toggle("is-off", !row.visible);
    el.innerHTML = `
      <span class="legend-direct-check">✓</span>
      ${row.swatch}
      <span>${row.label}</span>
    `;
